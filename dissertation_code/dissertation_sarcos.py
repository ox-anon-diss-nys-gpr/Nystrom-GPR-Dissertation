...
if __name__ == "__main__":
    # === Load and preprocess SARCOS dataset ===
    print("Loading data...")
    ...
    X_train = data_dict['sarcos_inv'][:, :21]
    y_train = data_dict['sarcos_inv'][:, 21]
    X_test = test_data_dict['sarcos_inv_test'][:, :21]
    y_test = test_data_dict['sarcos_inv_test'][:, 21]
    ...
    cv_folds = 3
    num_trials = 10
    y_std = 1  # Targets are already standardised

    # Subsample for fast CV
    subset_size = 10000
    X_sub = X_train_scaled[:subset_size]
    y_sub = y_train_scaled[:subset_size]

    # === Cross-validate bandwidth & noise ===
    print("\nCross-validating hyperparameters...")

    uniform_noise, uniform_bandwidth = cv_sampling_method(
        X_sub, y_sub, ranks, noise_candidates, bandwidth_candidates, method='uniform', cv_folds=cv_folds
    )
    rpcholesky_noise, rpcholesky_bandwidth = cv_sampling_method(
        X_sub, y_sub, ranks, noise_candidates, bandwidth_candidates, method='rpcholesky', cv_folds=cv_folds
    )
    rrls_noise, rrls_bandwidth = cv_sampling_method(
        X_sub, y_sub, ranks, noise_candidates, bandwidth_candidates, method='rrls', cv_folds=cv_folds
    )
    linear_kmeans_noise, linear_kmeans_bandwidth = cv_sampling_method(
        X_sub, y_sub, ranks, noise_candidates, bandwidth_candidates, method='linear_kmeans', cv_folds=cv_folds
    )
    ...
    plt.xlabel("Rank")
    plt.ylabel("RMSE")
    plt.title("RMSE vs Rank (SARCOS Dataset)")
    plt.legend()
    plt.show()
    ...
    plt.title("Sampling Time vs Rank (SARCOS Dataset)")
    plt.legend()
    plt.show()
    ...
    plt.title("GPR Time vs Rank (SARCOS Dataset)")
    plt.legend()
    plt.show()
    ...
    plt.title("Runtime vs Rank (SARCOS Dataset)")
    plt.legend()
    plt.show()

    # === Trace error evaluation ===
    print("\nEvaluating relative trace error...")

    bandwidth = 6.0  # Selected based on previous inspection
    ...
    plt.yscale('log')
    plt.title("Relative Trace Error vs Rank (SARCOS Dataset)")
    plt.legend()
    plt.show()